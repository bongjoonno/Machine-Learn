from src.machine_learn.imports import StandardScaler, LinearRegression, TabPFNRegressor, plt
from src.machine_learn.data_prep import baseline_model_regression_test_data_dict
...
def test_baseline_models(optimizer: LinearRegression | TabPFNRegressor, 
                         data_type: str,
                         scale_y: bool = False):

    if data_type not in baseline_model_regression_test_data_dict:
        raise ValueError(f'data_type must be in {baseline_model_regression_test_data_dict.keys()}')
    else:
        test_data = baseline_model_regression_test_data_dict[data_type]
